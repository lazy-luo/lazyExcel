...
BOOL CEditToolBar::CreateBar(CWnd* pParentWnd)
{
	...
	*/
	if (!CreateEx(pParentWnd, TBSTYLE_FLAT, 
		m_style_toolbar & (~CBRS_SIZE_DYNAMIC) | CBRS_SIZE_FIXED,
...
void CEditToolBar::OnSize(UINT nType, int cx, int cy) 
{
	...
	if(cx>0){
		GetItemRect(INDEX_VALUE, &rect);
		//LONG wid=(cx-rect.left+nDeff<MIN_WIDTH?MIN_WIDTH:cx-rect.left+nDeff);
...
void CEditToolBar::OnUpdateData()
{
	...
	if(pView){
		...
		if(pGrid){
			...
			CString strName=GetTextMatrix(sheet,nRow,nCol);
			CString str;
