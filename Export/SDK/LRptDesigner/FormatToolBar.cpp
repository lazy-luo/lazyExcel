...
{
	if (!CreateEx(pParentWnd, TBSTYLE_FLAT, m_style_toolbar,
...
void CFormatToolBar::OnUpdateColor()
{
	...
	if(pView){
		...
		if(pGrid){
			...
			LONG lngBorderClr=GetCurrentCellBorderColor(sheet);
...
void CFormatToolBar::OnUpdateCombo()
{
	...
	if(pView){
		...
		if(pGrid){
			LONG sheet=GetCurrentSheet(GetBook(pGrid));
			LONG nRow=GetFocusRow(sheet),nCol=GetFocusCol(sheet);
			CString strName=GetFontName(sheet,nRow,nCol);
			CString str;
			m_lstFontName.GetWindowText(str);
			if (str != strName)
				m_lstFontName.SetWindowText(strName);
			LONG nHeight=GetFontSize(sheet,nRow,nCol);
			CString strSize;
			if (nHeight < 0)
